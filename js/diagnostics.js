// diagnostics.js - simple HUD with FPS, XR, position, and module status.
...
export function createDiagnostics(el){
  ...
  function tick(dt, renderer, rig){
    ...
    const xr = renderer.xr?.isPresenting ? 'presenting' : 'ready';
    set('fps', state.fps);
    set('xr', xr);
    if (rig?.root){
  ...
  function render(){
    ...
    for (const [k,v] of state.map.entries()){
      rows.push(`<div><b>${escapeHtml(k)}</b>: ${escapeHtml(String(v))}</div>`);
    }
